...
**Status:** Implemented through `v1.2.0` — living spec  
**Owner:** Zolile Nonzapa  
**Purpose:** Single source of truth for how Mintry's dashboard, SDK, and sidecar fit together.  
**Companion docs:** [ARCHITECTURE.md](./ARCHITECTURE.md), [PHASE2_PLAN.md](./PHASE2_PLAN.md), [ROADMAP.md](./ROADMAP.md), [DEPLOYMENT.md](./DEPLOYMENT.md)
...
| §3.1 Auth UI | Supabase Auth email/password + magic link for dashboard; admin token break-glass | **Approved — implemented (v1.2.0)** |
...
```
   ...
   ┌─────────────────────────────────────────────────────────────────┐
   │  Dashboard (Next.js)     Auth (Supabase Auth UI + admin          │
   │  Sign & Push / Fleet     token break-glass)                      │
...
| Layer | Choice | Notes |
| ----- | ------ | ----- |
| Frontend + API routes | Vercel / Next.js | Sign, fleet, org compile, alias validate, Auth UI |
| Auth + relational data | Supabase Auth | Email/password + magic link (`/login`); admin token break-glass; optional `MINTRY_DASHBOARD_ALLOWED_EMAILS` |
...
### Auth UI (`v1.2.0`) — complete

- [x] Full Supabase Auth UI — email/password, magic link; admin token break-glass
- [x] Optional `MINTRY_DASHBOARD_ALLOWED_EMAILS`

### Next (see [ROADMAP.md](./ROADMAP.md))

1. Sidecar HTTPS MITM
2. Sidecar built-in policy poller (verify → LKG → flat caps)
3. Configurable intent blocklists in signed policy
4. Option B fleet hard cap (only if Option A insufficient)
5. Profiles / org RBAC beyond email allowlist
