...
- **Use `mintry.mandate()` for auto-attribution** — nested LLM calls inside the block need no `X-Mintry-Mandate` header (v1.3.0+).
- **Default mandate is `MINTRY_AGENT_ID` / `default_agent`** at `MINTRY_DEFAULT_BUDGET_USD` — not the legacy $0.01 trap.
- The dashboard `GET /api/summary` route merges Supabase `telemetry_events` when the control plane is configured. When unreachable, expect a delay before fallback to local ledger data.
